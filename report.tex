...
Clearly, extrapolation of Amdahl's Law does not provide an accurate predictor for our parallel speedup. Amdahl's Law relies on $f$, a fraction of the code which cannot be parallellized and runs serially. But our computation and initialization routines are completely parallel. What prevents the algorithm from being embarrassingly parallell is the communication between neighbouring processes which must occur every iteration. Thus Amdahl's model may be inappropriate for our problem, unless we allow $f$ to include communication, context switching and other technical factors. We proceed instead to examine the communication and initialization effort, in search of speedup limiting factors.
