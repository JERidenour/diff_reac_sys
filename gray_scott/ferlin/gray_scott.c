...
int main(int argc, char *argv[])
{
	...
	if (Psq == 100) {
		...
		int r = 20;
		if (rank == 44 || rank == 45 || rank == 54 || rank == 55) {
	...
	for (int i = 0; i < MAXITER; i++) {
		...
		// Uncomment when using serial version:
		// for (int i = 0; i < 2 * n_inner; i++) {
		// 	bin_north[i] = bout_north[i];
		// 	bin_south[i] = bout_south[i];
		// 	bin_east[i] = bout_east[i];
		// 	bin_west[i] = bout_west[i];
		// }
...
//==========================
	...
	double sum = tInit + tComm + tCalc;
	printf("Process %d, t_init: %f, t_comm: %f, t_calc: %f,\ntotal: %f.\n", \
	       rank, tInit, tComm, tCalc, sum);
	printf("hx: %f\n",hx);
